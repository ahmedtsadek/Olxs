...
class PS_Adapter {
	...
	function convert_user( &$obj , $need_return = false)
	{
		...
		// active item count
		$active_conds ['added_user_id'] = $obj->user_id;
		$active_conds['status'] = 1;
		$obj->active_item_count = $this->CI->Item->count_all_by($active_conds);
	...
	function convert_follow_user_list( &$obj, $followed_user_id )
	{
		...
		if ( is_array( $obj )) {
			...
			for ($i=0; $i < count($obj) ; $i++) { 
				if($obj[$i]->role_id == 4){
					$obj[$i]->post_count = 'limited';
				}else{
					$obj[$i]->post_count = 'unlimited';
				}
		
				// active item count
				$active_conds ['added_user_id'] = $obj[$i]->user_id;
				$active_conds['status'] = 1;
				$obj[$i]->active_item_count = $this->CI->Item->count_all_by($active_conds);
