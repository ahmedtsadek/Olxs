	...
	function save( $id = false ) {
		...
		if($data['status'] == 1) {
			...
			// to send subscribe noti who subscribe the category - start
				...
				if($updated_flag == 0){
					$sub_cat_id =  $this->Item->get_one($id)->sub_cat_id;
					$name = $this->Subcategory->get_one($sub_cat_id)->name;
					$subscribe_msg = get_msg( 'new_item_upload_label' ) . ' ' . $name;
		...
		$cond_dev['user_id'] = $user_id;

		$devices = $this->Noti->get_all_by($cond_dev)->result();
